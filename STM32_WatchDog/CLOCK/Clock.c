...
void TIM1_EnableClock(const unsigned char Status)
{
  if(Status==Enable) RCC->RCC_APB2ENR |= (1u<<11);
  else RCC->RCC_APB2ENR &= ~(1u<<11);
}
...
void TIM4_EnableClock(const unsigned char Status)
{
  if(Status==Enable) RCC->RCC_APB1ENR |= (1u<<2);
  else RCC->RCC_APB1ENR &= ~(1u<<2);
}

void TIM5_EnableClock(const unsigned char Status)
{
  if(Status==Enable) RCC->RCC_APB1ENR |= (1u<<3);
  else RCC->RCC_APB1ENR &= ~(1u<<3);
}

void TIM6_EnableClock(const unsigned char Status)
{
  if(Status==Enable) RCC->RCC_APB1ENR |= (1u<<4);
  else RCC->RCC_APB1ENR &= ~(1u<<4);
}

void TIM7_EnableClock(const unsigned char Status)
{
  if(Status==Enable) RCC->RCC_APB1ENR |= (1u<<5);
  else RCC->RCC_APB1ENR &= ~(1u<<5);
}
